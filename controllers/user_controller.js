  ...
  .then(
    function(err){
      if (err) {
        ...
        .save( {fields: ["username", "password"]})
        .then( function(){req.session.user.username=req.body.user.username; res.redirect('/');});//añadidopor mi lo de cambiar el username al nuevo para que se vea el acutal y no el viejo
      }     // Redirección HTTP a /
    }
  ).catch(function(error){next(error)});
};
